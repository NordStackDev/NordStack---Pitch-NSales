...
const Landing = () => {
  ...
  // Lokal opdatering: Hent brugerens egne stats når userProfile eller triggerRefresh ændrer sig
  ...
  const handleLogPitch = async () => {
    ...
    try {
      ...
      toast("Log pitch bekræftet", {
        description: "Din pitch er nu registreret.",
        duration: 2500,
        className:
          "bg-green-500 text-white font-semibold text-base shadow-lg border border-green-600",
      });
      // Stats opdateres nu lokalt efter logning
      triggerRefresh();
      await fetchMyStats();
    } catch (error: any) {
      console.error("Pitch insert error", error);
      toast("Fejl ved logning af pitch", {
        description: "Noget gik galt. Prøv igen eller kontakt support.",
        duration: 3500,
        className:
          "bg-red-100 border-red-400 text-red-900 font-semibold text-base shadow-lg",
      });
    } finally {
  ...
  const handleLogSale = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast("Log salg bekræftet", {
        description: "Dit salg er nu registreret.",
        duration: 2500,
        className:
          "bg-green-500 text-white font-semibold text-base shadow-lg border border-green-600",
      });
      ...
      toast("Fejl ved logning af salg", {
        description: "Noget gik galt. Prøv igen eller kontakt support.",
        duration: 3500,
        className:
          "bg-red-100 border-red-400 text-red-900 font-semibold text-base shadow-lg",
        ...
        <div className="text-center space-y-8">
          <div className="space-y-4">
            <h2 className="text-3xl font-bold">
              Velkommen til salgsoversigten
            </h2>
            <p className="text-muted-foreground max-w-md mx-auto">
              Log pitches og salg hurtigt.
            </p>
          </div>
